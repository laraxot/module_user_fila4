...
=======
# PHPStan Fixes - User Module (Livello 9)

## Errori Risolti

### 1. SetDefaultRolesBySocialiteUserAction - Proprietà Undefined
**File**: `app/Actions/Socialite/SetDefaultRolesBySocialiteUserAction.php`
**Errore**: `Access to an undefined property $provider`
**Causa**: La proprietà `$provider` non era definita nella classe ma veniva utilizzata nel costruttore
**Soluzione**: Aggiunta proprietà `$provider` e parametro nel costruttore
...
class SetDefaultRolesBySocialiteUserAction
{
    public function __construct() {
        $this->domainAnalyzer = app(
            ...
            [
                'ssoProvider' => $this->provider, // ERRORE: $this->provider non definito
            ]
        );
    }
}
...
class SetDefaultRolesBySocialiteUserAction
{
    ...
    public function __construct(string $provider = 'google') {
        ...
        $this->domainAnalyzer = app(
            EmailDomainAnalyzer::class,
            [
                'ssoProvider' => $this->provider, // OK: proprietà definita
            ]
        );
    }
}
...
**Motivazione**: Il provider SSO deve essere passato come parametro al costruttore per essere utilizzato nella configurazione dell'EmailDomainAnalyzer.

### 2. EmailDomainAnalyzer - Parametro Undefined
**File**: `app/Actions/Socialite/Utils/EmailDomainAnalyzer.php`
**Errore**: `Variable $ssoProvider in empty() is never defined`
**Causa**: Il parametro `$ssoProvider` non era definito nel costruttore ma veniva utilizzato
**Soluzione**: Aggiunto parametro `$ssoProvider` al costruttore e proprietà della classe
...
{
    public function __construct() {
        if (empty($ssoProvider)) { // ERRORE: $ssoProvider non definito
...
final class EmailDomainAnalyzer
{
    ...
    public function __construct(string $ssoProvider) {
        if (empty($ssoProvider)) { // OK: parametro definito
            throw new \InvalidArgumentException('Il provider SSO non può essere vuoto');
        }
...
**Motivazione**: Il provider SSO deve essere passato come parametro per essere utilizzato nei metodi di analisi del dominio email.

### 3. SetDefaultRolesBySocialiteUserAction - Tipo Collection vs Array
**File**: `app/Actions/Socialite/SetDefaultRolesBySocialiteUserAction.php`
**Errore**: `Parameter #1 $roles of method assignRole() expects array, Collection given`
**Causa**: Il metodo `assignRole()` si aspetta un array ma riceve una Collection
**Soluzione**: Convertito Collection in array usando `->toArray()`

```php
// PRIMA (ERRATO)
$userModel->assignRole($rolesToSet); // ERRORE: Collection invece di array

// DOPO (CORRETTO)
$userModel->assignRole($rolesToSet->toArray()); // OK: array convertito
```

**Motivazione**: Il metodo `assignRole()` di Spatie Permission si aspetta un array di ruoli, non una Collection.

### 4. BaseUser - Constructor Return Type Error
**File**: `app/Models/BaseUser.php`
**Errore**: `Method __construct() cannot declare a return type`
**Causa**: Il costruttore aveva un tipo di ritorno `void` esplicito
**Soluzione**: Rimosso il tipo di ritorno `void` dal costruttore

```php
// PRIMA (ERRATO)
public function __construct(): void { // ERRORE: costruttori non possono avere tipo di ritorno

// DOPO (CORRETTO)
public function __construct() { // OK: nessun tipo di ritorno
```

**Motivazione**: In PHP, i costruttori non possono dichiarare un tipo di ritorno esplicito.

### 5. BaseUser - Notifications Method Return Type Error
**File**: `app/Models/BaseUser.php`
**Errore**: `A void method must not return a value`
**Causa**: Il metodo `notifications()` era dichiarato come `void` ma restituiva una relazione Eloquent
**Soluzione**: Cambiato il tipo di ritorno da `void` a `\Illuminate\Database\Eloquent\Relations\MorphMany`

```php
// PRIMA (ERRATO)
public function notifications(): void {
    return $this->morphMany(Notification::class, 'notifiable'); // ERRORE: void method non può restituire valori
}

// DOPO (CORRETTO)
public function notifications(): \Illuminate\Database\Eloquent\Relations\MorphMany {
    return $this->morphMany(Notification::class, 'notifiable'); // OK: tipo di ritorno corretto
}
```

**Motivazione**: Il metodo `notifications()` restituisce una relazione Eloquent MorphMany, quindi deve avere il tipo di ritorno appropriato.

### 6. HasTeams - AddTeamMember Method Return Type Error
**File**: `app/Models/Traits/HasTeams.php`
**Errore**: `A void method must not return a value`
**Causa**: Il metodo `addTeamMember()` era dichiarato come `void` ma restituiva un modello
**Soluzione**: Aggiunto tipo di ritorno e parametri mancanti

```php
// PRIMA (ERRATO)
public function addTeamMember(): void {
    // ... logica ...
    return $teamUser; // ERRORE: void method non può restituire valori
}

// DOPO (CORRETTO)
public function addTeamMember(\Illuminate\Database\Eloquent\Model $user, ?\Illuminate\Database\Eloquent\Model $role = null): \Illuminate\Database\Eloquent\Model {
    // ... logica ...
    return $teamUser; // OK: tipo di ritorno corretto
}
```

**Motivazione**: Il metodo `addTeamMember()` restituisce il modello TeamUser creato e richiede parametri per funzionare correttamente.

## Pattern Identificati

### Socialite Actions
- Le azioni Socialite devono ricevere il provider come parametro
- Utilizzare proprietà readonly per configurazioni immutabili
- Gestire correttamente i tipi di dati tra Collection e array

### Eloquent Relations
- I metodi di relazione devono avere il tipo di ritorno corretto
- Non utilizzare `void` per metodi che restituiscono relazioni
- Utilizzare i tipi generici appropriati per le relazioni

### Constructor Patterns
- I costruttori non possono avere tipi di ritorno espliciti
- Utilizzare parametri per configurazioni necessarie
- Inizializzare proprietà readonly nel costruttore

## Collegamenti
- [README.md](./README.md)
- [Authentication Documentation](./authentication.md)
- [Socialite Integration](./socialite-integration.md)
...
### 5. Conflitto Git in ResetPasswordWidget
**File**: `app/Filament/Widgets/Auth/ResetPasswordWidget.php`
**Errore**: `Syntax error, unexpected T_SL on line 117` e `Syntax error, unexpected EOF on line 129`
**Causa**: Conflitto Git non risolto nel PHPDoc del metodo `resetPassword()`
**Soluzione**: Risolto il conflitto mantenendo la versione corretta con tipo di ritorno nullable
...
public function resetPassword(): ?\Illuminate\Http\RedirectResponse {
    ...
    return null; // Aggiunto per gestire il caso senza redirect
}
```

**Motivazione**: I conflitti Git devono essere sempre risolti prima di committare. Il metodo `resetPassword()` può restituire un redirect o null, quindi il tipo di ritorno deve essere nullable.

**Business Logic**: Il widget `ResetPasswordWidget` gestisce il reset delle password degli utenti attraverso l'interfaccia Filament. Implementa la logica di sicurezza di Laravel per il reset password con feedback appropriato all'utente.
