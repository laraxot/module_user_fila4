...
## PHPStan Fixes
- **DocBlock Updates**: Fixed `class.notFound` errors in `User` and `TenantUser` models by replacing non-existent `Meetup\Models\Profile` references with `Modules\Xot\Contracts\ProfileContract`.
